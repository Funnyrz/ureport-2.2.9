...
export function tableToXml(context) {
    ...
    for (let datasource of datasources) {
        ...
        if (type === 'jdbc') {
            ...
            $.ajax({
                url: window._server + "/designer/loadDataSourceInfo",
                type: 'POST',
                async:false,
                data: {
                    id:datasource.id
                ...
                success: function (result) {
                    ds = `<datasource name="${encode(datasource.name)}" type="${datasource.type}"`;
                    ds += ` username="${result.username}"`;
                    ds += ` password="${result.password}"`;
                    ds += ` url="${result.url}"`;
                    ds += ` driver="${result.driver}"`;
                    ds += '>';
                    for (let dataset of datasource.data) {
                        ds += `<dataset name="${encode(dataset.name)}" type="sql">`;
                        ds += `<sql><![CDATA[${dataset.sql}]]></sql>`;
                        for (let field of dataset.fields) {
                            ds += `<field name="${field.name}"/>`;
