...
export default class DatasourcePanel{
    ...
    buildPanel() {
        ...
        const addRemoteBtn=$(`<button class="btn btn-default" style="border:none;border-radius:0;background: #f8f8f8;padding: 6px 8px;" title="${window.i18n.property.datasource.addRemote}">
            <i class="ureport ureport-summary"></i>
        </button>`);
        toolbar.append(addRemoteBtn);
        this.remoteDialog=new RemoteDialog(this.datasources);
        addRemoteBtn.click(function(){
            _this.remoteDialog.show(function(name,beanId){
                const ds={name,beanId};
                const tree=new RemoteTree(_this.treeContainer,_this.datasources,ds,_this.remoteDialog,_this.context);
                _this.datasources.push(tree);
            });
        });
    ...
    buildDatasources() {
        ...
        for(let ds of this.datasources){
            if (ds.type === 'jdbc') {
                ...
                new RemoteTree(this.treeContainer, this.datasources, ds,this.remoteDialog, this.context);
            }
        }
    }
}