    ...
    _buildDatasource(id,name) {
        ...
        $.contextMenu({
            ...
            callback: function (key, options) {
                if (key === 'selectDataset') {
                    debugger
                    const span = $(options.selector);
                    $.ajax({
                        url: window._server + "/datasource/loadRemoteDataset",
                        type: 'POST',
                        data: {},
                        success: function (resp) {
                            ...
                            const rootSpan = $(`
                                <span id="${this.id}">
                                    <i class='ureport ureport-minus' style='margin-right:2px'></i>
                                    <i class="ureport ureport-shareconnection"></i> <a href='###' class="ds_name">${result[0].name}</a>
                                </span>`);
                            dialog(`${window.i18n.tree.selectDatasetTitle}`, rootSpan, function () {
            ...
            items: {
                ...
                "selectDataset": {
                    name: `${window.i18n.tree.selectDataset}`,
                    icon: "edit"
                },
                "delete": {
