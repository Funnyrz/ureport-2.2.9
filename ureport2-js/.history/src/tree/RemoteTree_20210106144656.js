...
export default class DatabaseTree extends BaseTree {
                ...
                for (let item of result.items) {
                    ...
                    if (id === _this.id) { 
                        ...
                        for (let dataset of _this.datasets) {
                            const data = { "name": dataset.name, "parameters": [], "sql": "select * from sys_user" }
    ...
    addDataset(dataset,code) {
        const li = $(`<li></li>`);
        const spanId = uuid.v1();
        const span = $(`<span id="${spanId}"><i class='ureport ureport-plus' style='margin-right:2px'></i> <i class="ureport ureport-sqlds"></i> <a href='###' class="dataset_name">${dataset.name}</a></span>`);
        li.append(span);
