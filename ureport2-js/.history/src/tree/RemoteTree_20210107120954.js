...
export default class DatabaseTree extends BaseTree {
    constructor(container, datasources, ds, remoteDialog, context) {
        super();
        this.type = 'remote';
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (let item of result.items) {
                    const id =  item.code;
                    if (id === _this.id) { 
                        _this.datasets = item.items
                        for (let dataset of _this.datasets) {
                            const data = { "id":dataset.code,"name": dataset.name, "parameters": [], "sql": "select * from sys_user" }
                            const fieldsUL = _this.addDataset(data,dataset.code);
                            _this.data.push(data);
                            _this.fieldsUL.push(fieldsUL);
                            // _this.buildFileds(data, fieldsUL);
                        }
                    }
                }
            },
            error:function(response){
                if(response && response.responseText){
                    ...
                    alert(`${window.i18n.dialog.remoteDS.loadFail}`);
                }
            }
        });
    }
    _buildDatasource() {
        ...
        const _this = this;
        const datasetDialog = new SqlDatasetDialog(this, {
            parameters: []
        });
        $.contextMenu({
            selector: '#' + this.id,
    ...
    addDataset(dataset,id) {
        ...
        const flagIcon = "ureport-plus";
        if ("dataSource" === dataset.parType) { 
            flagIcon = "ureport-minus";
        }
        const span = $(`<span id="${id}"><i class='ureport ${flagIcon}' style='margin-right:2px'></i> <i class="ureport ureport-sqlds"></i> <a href='###' class="dataset_name">${dataset.name}</a></span>`);
        li.append(span);
        ...
        span.click(function (e) {
            ...
            if ($liChildren.is(":visible")) {
                ...
                for (var i = 0; i < _this.data.length; i++) { 
                    const dataset = _this.data[i];
                    if (dataset.id === span[0].id) {
                        _this.buildFileds(dataset, _this.fieldsUL[i]);
